...
class NeuralNetController():
    ...
    def update_controller(self, error, delerror_delt, sumerror, target):
        self.backward_propagation(error, delerror_delt, sumerror, target)
    
    def backward_propagation(self, error, delerror_delt, sumerror, target): # where prediction_error is an array of the errors between the plant output and the desired output in an epoch
        print("mse: ", self.compute_mse(error, delerror_delt, sumerror, self.weights, self.biases, target))#0, 0, 0, self.weights, self.biases, 1))
        ...
        self.update(delmse_delomega(error, delerror_delt, sumerror, self.weights, self.biases, target)[0], delmse_delomega(error, delerror_delt, sumerror, self.weights, self.biases, target)[1])
        print("weights: ", self.weights)
        print("biases: ", self.biases)
...
#for _ in range(5):
for i in range(5):
    target = jnp.float32(i + 1)
    print("target: ", target)
    pred1 = nn1.compute_control_signal(0,0,0)
    #print(jnp.abs(pred1 - 1)[0])
    #print(pred1)
    nn1.update_controller(jnp.abs(pred1 - target)[0][0], 0.0, jnp.abs(pred1 - target)[0][0], target)